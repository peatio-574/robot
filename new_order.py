...
def get_order(flag='imgextra/i3'):
    ...
    orders = dict()
    time.sleep(5)
    pages = int(Playwright_.page.locator('(//button/span[@class="next-btn-helper"])[14]').text_content())  # 页数
...
def add_speecial_product():
    add_to_purchase('u0000000074588', 'u0000000074588025', 1)

def delelte_speecial_product(uniqo_buy_id):
    product = '486121'
    list_url = 'https://www.uniqlo.cn/account/order/order_list.html'
    Playwright_.goto(list_url)
    time.sleep(3)
    refund_ele = f'//tr[@id="{uniqo_buy_id}"]/following-sibling::tr//a[contains(text(), "{product}")]/../../..//a[text()="申请退款"]'
...
def main(controller, devive_ip, is_ship):
    ...
    for order in orders:
        ...
        if not uniqo_buy_id:
            ...
            delelte_speecial_product(uniqo_buy_id)
        # 备注
        if status:
...
def zfb_pay(controller, device_ip, server='127.0.0.1', rotation=3, finish=False):
    ...
    logger.info('点击返回')
    controller.move_click(device_ip=device_ip, x_ratio=0.08304, y_ratio=0.06057, home=False)
...
if __name__ == '__main__':
    ...
    for i in range(number):
        main(controller, devive_ip, is_ship)
